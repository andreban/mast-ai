...
describe('BuiltInAIAdapter', () => {
  ...
  describe('generate', () => {
    ...
    it('always returns empty toolCalls', async () => {
      const response = await adapter.generate({
        messages: [{ role: 'user', content: { type: 'text', text: 'Hi' } }],
        tools: [
          { name: 'myTool', description: 'does stuff', parameters: {}, scope: 'read' as const },
        ],
      });
    ...
    it('throws AdapterError when context window is full after session creation', async () => {
      mockCreate.mockResolvedValue(makeSession({ contextUsage: 1000, contextWindow: 1000 }));
  ...
  describe('generateStream', () => {
    async function collect(request: Parameters<BuiltInAIAdapter['generateStream']>[0]) {

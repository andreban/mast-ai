...
- `demos/core/basic-chat/` — A Vite-powered frontend demonstrating a Hybrid Mode chat agent with local tools.
- `demos/built-in-ai/prompt-api/` — A Vite-powered frontend demonstrating on-device inference via the browser Prompt API.
- `demos/built-in-ai/summarizer/` — A Vite-powered frontend demonstrating the `SummarizeTool` backed by the browser Summarizer API.
- `demos/built-in-ai/translate/` — A Vite-powered frontend demonstrating the `TranslateTool` backed by the browser Translator API.
- `demos/core/rust-server/` — A sample URP reasoning engine backend written in Rust (Axum + async channels).
...
1. **Start the reasoning backend (Rust):**
   ```bash
   cd demos/core/rust-server
   cargo run
   ```
2. **Start the frontend (in a new terminal):**
   ```bash
   cd demos/core/basic-chat
   npm run dev
   ```
